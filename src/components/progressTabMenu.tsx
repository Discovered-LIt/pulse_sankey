...
const ProgressTabMenu = ({
  ...
  useEffect(() => {
    ...
    const container: any = mainContainerId ? document.getElementById(mainContainerId) : window;
    ...
    const handleScroll = () => {
      ...
      contents.forEach((content: HTMLDivElement, idx) => {
        const conatinerScrollTop = (container?.scrollTop || container?.scrollY) + 110
        if (
          conatinerScrollTop >= content.offsetTop &&
          conatinerScrollTop < content.offsetTop + container.clientHeight
        ) {
          newActiveTab = idx;
          const scrollProgress = (conatinerScrollTop - content.offsetTop) / content.clientHeight;
        ...
        options.map((option: string, idx) => (
          <div className={cn([
            ...
            <div className={cn([
              ...
              <div className="bg-white h-[2px] rounded" style={{width: `${scrollPercentage}%`}}></div>
            </div>
          </div>
        ))
      }
